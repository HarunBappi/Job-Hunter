...
const JobDetails = () => {
  ...
  const { _id, title, date, category, minPrice, maxPrice, description, buyer } =
    job || {};
  ...
  const handleBids = async (e) => {
    ...
    if (compareAsc(new Date(), new Date(date)) === 1)
      return toast.error("Deadline crossed! Bidding Another Jobs");

    // Price in With maximum price range Validation
    if (price > maxPrice)
      return toast.error(
        "Your Offer price less or Atleast equal to maximum Price!"
      );

    // Offered Deadline is crossed Seller Deadline validation
